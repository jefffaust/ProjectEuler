...
Row readRow(std::ifstream& ifstr, int rowSize)
{
	Row row(rowSize, 0);
...
Triangle read(int rows)
{
	std::ifstream ifstr("triangle.txt");
	Triangle t;
	for( int row = 0; row < rows; ++row )
	{
		t.push_back(readRow(ifstr, row + 1));
	}
	return t;
}
...
void collapse(Triangle& triangle, int row)
{
	...
	{
		triangle[row][i] += std::max(triangle[row+1][i], triangle[row+1][i+1]);
	}
}
...
int main()
{
	...
	return result(1074, triangle[0][0]);
}
